...
class Actions
  def self.execute(status, action)
    case action
      when 1..7 then action.each { |field, key| status[field] = status[field].send key['operator'],key['value'].to_i }
      when 8 then Saver.save_load_menu(status, 7)
      when 9 then Saver.save_load_menu(status, 8)
      when 0 then exit
  end
